        ...
        <table className="min-w-full text-sm border shadow rounded bg-white">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="p-2">Price CAGR</th>
              <th className="p-2">Piotroski</th>
              <th className="p-2">X Return</th>
          ...
          <tbody>
            {filtered.map(stock => (
              <tr key={stock.symbol} className="border-t hover:bg-gray-50">
                ...
                <td className="p-2">{stock.classification?.join(', ')}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {filtered.length === 0 && <p className="text-center p-4 text-gray-500">No stocks match the filters.</p>}
      </div>
    </div>
  );
}
